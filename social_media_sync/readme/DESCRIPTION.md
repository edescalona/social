...
- Comment thread of a publication, read from the dashboard: a comment is
  ...
  the social media serves the replies, and *Recommend* toggles the reaction of
  the account on the publication and on each of its comments. Which of them a
  social media really serves is declared by its connector, and only then is
  the entry offered.
