...
class SocialPost(models.Model):
    ...
    def create(self, vals_list):
        ...
        posts._anchor_media_attachments()
        return posts
    ...
    def unlink(self):
        ...
        The medias go with it, and nothing here has to arrange that:
        :meth:`_anchor_media_attachments` attaches every media to the record
        that holds it, and ``unlink`` deletes the attachments of the records
        it deletes, so the images of the post leave with the post and what a
        publication downloaded leaves with the publication. The filestore
        frees the files on its next collection.
        """
    ...
    def _prepare_post_account_values(self):
        ...
        Every publication points at the very attachments of the post: it
        shares them instead of keeping a copy of its own, so a post published
        on several accounts stores one image once. What each account made of
        that image on its side is written in ``media_refs``, which is per
        publication and not per attachment.
