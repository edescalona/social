...
class TestSocialAccountBase(TestSocialMediaBaseCommon):
    ...
    def test_purge_account_leaves_no_media_behind(self):
        """Deleting an account and its history leaves no attachment behind.

        Both kinds go: the media the post carries, which its publication
        shares, and the one the publication downloaded from the social
        media. Every one of them is attached to the record that holds it,
        which is what makes ``unlink`` take it along.
        """
        Attachment = self.env["ir.attachment"]
        post = self.social_post_id
        post_account = self.social_post_account_id
        shared = Attachment.create(
            {"name": "purged_shared.png", "datas": self.image_base64}
        )
        post.write({"image_ids": [Command.set(shared.ids)]})
        post_account.write({"image_ids": [Command.set(shared.ids)]})
        downloaded = Attachment.create(
            {
                "name": "purged_downloaded.png",
                "datas": self.image_base64,
                "res_model": "social.post.account",
                "res_id": post_account.id,
            }
        )
