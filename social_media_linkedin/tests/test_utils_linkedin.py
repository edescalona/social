...
ENCODED_QUERY_PARAMETERS = [
    # social_media_linkedin/models/social_account.py:248 _refresh_token()
    (
        "linkedin:248 grant_type",
        "grant_type",
        "refresh_token",
        "grant_type=refresh_token",
    ),
    (
        "linkedin:248 refresh_token",
        "refresh_token",
        _TOKEN,
    ...
    (
        "linkedin:248 client_secret",
        "client_secret",
        _CLIENT_SECRET,
        "client_secret=test_client_secret_aB3cD4%3D%3D",
    ),
    # :316 / :372 the image and the video upload.
    (
        "linkedin:316 action",
        "action",
        "initializeUpload",
    ...
    # :1027 _get_posts(), the finder and the batch get.
    (
        "linkedin:1027 q",
        "q",
        "author",
    ...
    (
        "linkedin:1027 sortBy",
        "sortBy",
        "LAST_MODIFIED",
        "sortBy=LAST_MODIFIED",
    ),
    (
        "linkedin:1027 ids one share",
        "ids",
        [_SHARE_URNS[0]],
        "ids=List(urn%3Ali%3Ashare%3A6912345678901234567)",
    ),
    (
        "linkedin:1027 ids one ugcPost",
        "ids",
        [_UGC_URNS[0]],
        "ids=List(urn%3Ali%3AugcPost%3A7132564752928563200)",
    ),
    # :1085 the only list holding several separate elements.
    (
        "linkedin:1085 ids images",
        "ids",
        _IMAGE_URNS,
        "ids=List(urn%3Ali%3Aimage%3AD4E10AQHxYzKq9pLmNw,urn%3Ali%3Aimage%3AD4E10AQFbT"
        "t3sVn2QkA)",
    ),
    # social_media_linkedin_sync/models/social_account.py:148
    # _get_entity_share_statistics().
    (
        "linkedin_sync:148 organizationalEntity",
        "organizationalEntity",
        _ORG_URN,
        "organizationalEntity=urn%3Ali%3Aorganization%3A96482531",
    ),
    (
        "linkedin_sync:148 shares",
        "shares",
        [",".join(_SHARE_URNS)],
    ...
    (
        "linkedin_sync:148 ugcPosts",
        "ugcPosts",
        [",".join(_UGC_URNS)],
        "ugcPosts=List(urn%3Ali%3AugcPost%3A7132564752928563200,urn%3Ali%3AugcPost%3A7"
        "132564752928563201)",
    ),
    # social_media_linkedin_sync/models/social_account.py:245
    # _get_ugc_posts_statistics(), the /socialActions batch get.
    (
        "linkedin_sync:245 ids ugcPosts",
        "ids",
        [",".join(_UGC_URNS)],
        "ids=List(urn%3Ali%3AugcPost%3A7132564752928563200,urn%3Ali%3AugcPost%3A713256"
        "4752928563201)",
    ),
    # :1360 _get_linkedin_daily_statistics(). The Rest.li struct that used
    # to need an exception of its own so that its colons survived.
    (
        "linkedin:1360 timeIntervals",
        "timeIntervals",
...
class TestBatchUrnsByUrlSize(TestSocialCommonLinkedin):
    ...
    A pure function of ``social_linkedin_utils``, tested here rather than
    through the account that happens to call it.
    """
