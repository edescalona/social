...
class TestSocialMediaLinkedin(TestSocialCommonLinkedin):
    ...
    def test_get_linkedin_scopes_asks_only_for_what_it_calls(self):
        """The connector asks for the permissions its own calls consume.

        The test runs ``at_install``, so the connectors depending on this one
        are not loaded yet and the list is the one this module contributes.
        """
        self.assertEqual(
    ...
    def test_get_linkedin_scopes_leaves_the_optional_ones_out(self):
        """The scopes no call consumes are declared but never requested."""
        scopes = self.media_linkedin_id._get_linkedin_scopes()
        self.assertEqual(len(_SCOPE_OPTIONAL_LINKEDIN), 6)
        for scope in _SCOPE_OPTIONAL_LINKEDIN:
            self.assertNotIn(scope, scopes)
            self.assertNotIn(scope, _SCOPE_LINKEDIN)
