...
- Nothing walks the feed to do it: Odoo already knows the URN of every
  ...
  **three calls** — `organizationalEntityShareStatistics` once for the shares
  and once for the UGC posts, and `socialActions` for the likes and the
  comments, which LinkedIn documents as the up-to-date ones.
